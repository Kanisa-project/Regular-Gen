...
class ARTAY(ttk.LabelFrame):
    def __init__(self, width, height, master=None, idutc_frame=None):
        ...
        self.glythTab = tab_glyth.Glyther(master=self.tabControl, idutc=self.IDUTC_frame,
                                          width=int(width), height=int(height))
        self.glyphTab = tab_glyph.Glyphin(master=self.tabControl, idutc=self.IDUTC_frame,
                                          width=int(width), height=int(height))
        self.spiriteTab = tab_spirite.Spirite(master=self.tabControl, idutc=self.IDUTC_frame,
                                              width=int(width), height=int(height))
        self.wordieTab = tab_wordie.Wordie(master=self.tabControl, idutc=self.IDUTC_frame,
                                           width=int(width), height=int(height), kinvow_size=(620, 940))
        self.recipeTab = tab_recipe.AlaNFT(master=self.tabControl, idutc=self.IDUTC_frame,
                                           width=int(width), height=int(height))
        self.fotoTab = tab_foto.Fotoes(master=self.tabControl, idutc=self.IDUTC_frame,
                                       width=int(width), height=int(height))
        self.mujicTab = tab_mujic.Mujic(master=self.tabControl, idutc=self.IDUTC_frame,
                                        width=int(width), height=int(height))
        self.gaimTab = tab_gaim.Gaim(master=self.tabControl, idutc=self.IDUTC_frame,
                                     width=int(width), height=int(height))
        self.meemTab = tab_meem.Meem(master=self.tabControl, idutc=self.IDUTC_frame,
                                     width=int(width), height=int(height))
        ...
        self.texioty_commands = {
            "glyth": [self.change_glyth, "Add glyth options to the kre8dict.",
                      {"add": "Add a glyth to the kre8dict.",
                       "new": "Make a new option for glyth."}, "ARTY", theme.rgb_to_hex(theme.LIGHT_GOLDENROD_YELLOW),
                      theme.rgb_to_hex(theme.DARK_SEA_GREEN)],
            "glyph": [self.change_glyph, "Add glyph options to the kre8dict.",
                      {"add": "Add a glyph to the kre8dict.",
                       "new": "Make a new option for glyph."}, "ARTY", theme.rgb_to_hex(theme.LIGHT_GOLDENROD_YELLOW),
                      theme.rgb_to_hex(theme.DARK_SEA_GREEN)],
            "wordie": [self.change_wordie, "Change wordie options and choices.",
                       {"add": "Add a wordie type to the kre8dict.",
                        "font": "Select which font for Kinvow to use."}, "ARTY", theme.rgb_to_hex(theme.LIGHT_GOLDENROD_YELLOW),
                       theme.rgb_to_hex(theme.DARK_SEA_GREEN)],
            "recipe": [self.change_recipe, "Change recipe options and choices.",
                       {"add": "Add a recipe to the kre8dict.",
                        "new": "Make a new recipe for the book."}, "ARTY", theme.rgb_to_hex(theme.LIGHT_GOLDENROD_YELLOW),
                       theme.rgb_to_hex(theme.DARK_SEA_GREEN)],
            "spirite": [self.change_spirite, "Change spirite options and choices.",
                        {"add": "Add a spirite to the kre8dict.",
                         "new": "Import a new set of spirite layers."}, "ARTY", theme.rgb_to_hex(theme.LIGHT_GOLDENROD_YELLOW),
                        theme.rgb_to_hex(theme.DARK_SEA_GREEN)]
        }
