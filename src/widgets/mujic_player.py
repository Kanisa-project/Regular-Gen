    ...
    def __init__(self, width, height, master=None, idutc_frame=None):
        ...
        self.texioty_commands = {
            "play": [self.play_loaded_sound, "Plays whatever sound is loaded.",
                     {}, "MUJC", theme.rgb_to_hex(s.LIGHT_WOOD), theme.rgb_to_hex(s.VERY_DARK_BROWN)]
        }

        self.loaded_sound = pygame.mixer.Sound(f"assets\Sounds\sound{random.randint(0, 9)}.wav")
        self.loaded_sound2 = pygame.mixer.Sound(f"assets\Sounds\sound{random.randint(0, 9)}.wav")
