...
def download_goblins():
    ...
    for card in random_goblins:
        ...
        if card.image_url is not None:
            ...
            with open(f'/home/trevor/Documents/PycharmProjects/KanisaBot/config/cardsMagic/{save_name}.png',
                      'wb') as handler:
...
#     imaj.save(f"depictions\\playmats\\{save_name}.png")
...
def depict_spell(img: Image, spell_info_dict: dict) -> Image:
    ...
    # img.save(f"depictions\\{spell_name}.png")
    return img
    ...
    for cost_point in spell_polypoints['cost_points']:
        ...
        for color in cost_slot['medium']:
            ...
            draw.line((cost_point, (img.size[0] // 2, img.size[1] // 2)),
                      fill=color,
                      width=int(spell_info_dict['spell_cmc']) + (
                              7 - (s.s.clamp(cost_slot['medium'].index(color), 0, len(cost_slot['medium']) - 1) * 4)))
    #
...
def get_id_color(color_id: str) -> tuple|None:
    if "R" in color_id:
        return s.CRIMSON
    if "G" in color_id:
        return s.FOREST_GREEN
    if "U" in color_id:
        return s.NAVY_BLUE
    if "W" in color_id:
        return s.GHOST_WHITE
    if "B" in color_id:
        return s.BLACK
    return None
...
def depict_name(spell_name: str, image_size: tuple) -> list:
    ...
    for x in range(0, image_size[0], 3):
        planned_name_lines.append(plan_angled_line(x, 0, 90, name_len, 1, s.BLUE_2, (image_size[0], image_size[1])))
        planned_name_lines.append(
            plan_angled_line(x, image_size[1], 270, name_len, 1, s.BLUE_2, (image_size[0], image_size[1])))
    for y in range(0, image_size[1], 3):
        planned_name_lines.append(plan_angled_line(0, y, 0, name_len, 1, s.BLUE_2, (image_size[0], image_size[1])))
        planned_name_lines.append(
            plan_angled_line(image_size[0], y, 180, name_len, 1, s.BLUE_2, (image_size[0], image_size[1])))
...
#     if os.path.isdir(f"depictions/{depicted_dict['spell_set'].upper()}/"):
#         save_path = f"depictions/{depicted_dict['spell_set'].upper()}/{save_name}.png"
#     else:
#         os.mkdir(f"depictions/{depicted_dict['spell_set'].upper()}")
#         save_path = f"depictions/{depicted_dict['spell_set'].upper()}/{save_name}.png"
...
class MagicAPIHelper(BaseAPIHelper):
    ...
    def add_card_database(self, new_card):
        ...
        self.db_helper.execute_query(all_card_insert_query, [new_card.name, new_card.rarity, new_card.type, "Magic the Gathering", new_card.set+'-'+new_card.number])

    def download_card_batch(self, batch_config: dict):
        super().download_card_batch(batch_config)
        all_type_cards = []
        current_cards = mtgsdk.Card.where(set=self.batch_set_id).where(type=self.batch_type).where(colors=self.batch_colors).all()
        for card in current_cards:
            all_type_cards.append(card)
            self.add_card_database(card)
        # print(all_type_cards)
        try:
            chosen_cards = random.sample(all_type_cards, self.batch_size)
        except ValueError as e:
            print(e)
            print(all_type_cards)
            chosen_cards = random.sample(all_type_cards, len(all_type_cards))
...
class MtgDepicter(glythed.TcgDepicter):
